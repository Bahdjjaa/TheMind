...
void boucle_principale_client_robot(int sockfd, const char *nom_robot)
{
    ...
    while (1)
    {
        ...
        if (bytes_recus > 0)
        {
            ...
            else if (strstr(buffer, "Continuez") != NULL && strstr(buffer, "Manche réussie") == NULL)
            {
            ...
            if (index_min != -1)
            {
                double delai = 2 + (15.0 / 98) * (min_val - 1); // Calcul du délai
                printf("Le robot attend %.2f secondes avant de jouer la carte : %d\n", delai, cartes[index_min].num);
                time_t debut_attente = time(NULL);
                while (difftime(time(NULL), debut_attente) < delai)
                {
                    memset(buffer, 0, sizeof(buffer));
                    int check = recv(sockfd, buffer, sizeof(buffer) - 1, MSG_DONTWAIT); // Vérification non bloquante
                    if (check > 0)
                    {
                        buffer[check] = '\0';
                        if (strstr(buffer, "Manche échouée") != NULL || strstr(buffer, "terminer la partie") != NULL || strstr(buffer, "PARTIE TERMINÉE") != NULL)
                        {
                            printf("Manche échouée reçue pendant le délai : annulation de l'attente.\n");
                            break; // Sortir de la boucle d'attente
                        }
                    }
                }
                usleep(100000); // Petite attente pour limiter la charge CPU

                // Envoyer la carte au serveur sauf dans le cas où la manche n'est pas échouée avant l'envoi
                if (strstr(buffer, "Manche échouée") == NULL && strstr(buffer, "terminer la partie") == NULL && strstr(buffer, "PARTIE TERMINÉE") == NULL)
                {
